...
และภารกิจที่มีอยู่ตอนนี้ (`exMission` / `MISSION 1` / `MISSION 2` + รหัสคีย์แพดอีก 114 อัน)
ทำอะไรทีละขั้น
...
ลงทะเบียนไว้ใน `missionPrograms[]` ท้ายไฟล์ `mission_list.cpp` — ตอนนี้มี **117 รายการ**
...
| 99–116 | `1010` … `1133` | `p1010` … | รหัสคีย์แพด Mode 1 (18 อัน) |
...
Mode 1 : mode(1) + field(0=BLUE,1=RED) + row(1..3)  + step(0,2,3)       → 4 หลัก
```
