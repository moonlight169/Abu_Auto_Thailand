...
- **ทิศ encoder ยังไม่ verify**: สมมติว่า pulse count เพิ่มขึ้นตอนยกขึ้น (zero อยู่ล่างสุดหลัง `setZero()`) ถ้า wiring จริงกลับทาง `liftTo()` จะไล่ตำแหน่งผิดทิศ (แต่ safety cutoff จะกันไม่ให้ชนจริง แค่พฤติกรรมจะแปลก) — ทดสอบด้วย `liftTo()` ค่าน้อยๆ ก่อนใช้งานจริง
- ค่า `LIFT_HOME_SPEED`, `LIFT_PID_KP/KI/KD_FRONT`, `LIFT_PID_KP/KI/KD_BACK` และ `LIFT_PULSE_TOLERANCE` ใน `lift.cpp` เป็นค่าตั้งต้น ต้อง tune กับของจริง
