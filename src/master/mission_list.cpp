// ===========================================================================
//  >>> THE MISSION SEQUENCES. THIS IS THE FILE TO EDIT FOR A NEW RUN. <<<
//
//  Each row is one step. The runner starts a row, waits until it reports
//  done, then moves to the next one. See mission_steps.h for the full list
//  of available *_STEP() builders and their arguments.
//
//  Add another mission by writing a new `const MissionStep missionN[]` array
//  and registering it in missionPrograms[] at the bottom.
// ===========================================================================

#include "mission_list.h"

#include <string.h>

/////////////////////////////////////////////////////////////////////////////////MissionStep/////////////////////////////////////////////////////////////////////////////////

// Add more mission programs by creating another MissionStep array and
// registering it in missionPrograms[] below.
const MissionStep exMission[] =
{
  RESET_STEP(),
  LIFT_STEP(100, 200),
  MOVE_STEP(3.8,0.5,0,20),
  LIFT_STEP(1000, 1100),
  WAIT_STEP(200),
  LIFT_STEP(2000, 2100),
  WAIT_STEP(200),
  LIFT_STEP(3800, 3900),
  WAIT_STEP(200),
  LIDAR_PARK_STEP(385, 30000),
  HUB_RELAY_STEP(1, 1),
  WAIT_STEP(500),
  ARM_BOTTOM_STEP(5),
  ARM_TOP_STEP(25),
  HUB_RELAY_STEP(2, 1),
  WAIT_STEP(500),
  ARM_TOP_STEP(100),
  WAIT_STEP(500),
  ARM_BOTTOM_STEP(180),
  HUB_RELAY_STEP(1, 0),
  WAIT_STEP(500),
  LIFT_STEP(0, 3900),
  LASER3_FORWARD_STEP(0.5,15000),
  LIFT_STEP(0, 0),

  LASER4_FORWARD_STEP(0.5,15000),
  LIFT_STEP(100, 200),

  FRONT_LIMIT_STEP(0.5f, 30000),
  LIFT_STEP(2000, 2100),
  LIDAR_PARK_STEP(385, 30000),
  HUB_RELAY_STEP(1, 1),
  WAIT_STEP(500),
  ARM_TOP_STEP(175),
  HUB_RELAY_STEP(3, 1),
  WAIT_STEP(500),
  ARM_TOP_STEP(100),
  WAIT_STEP(500),
  LIFT_STEP(0, 2100),
  LASER3_FORWARD_STEP(0.5,15000),
  LIFT_STEP(0, 0),
  LASER4_FORWARD_STEP(0.5,15000),
  RESET_STEP(),
  LIFT_STEP(100, 200),
  HUB_RELAY_STEP(1, 0),
  LIDAR_PARK_STEP(385, 30000),
  HUB_RELAY_STEP(1, 1),
  WAIT_STEP(500),
  ARM_BOTTOM_STEP(90),
  HUB_RELAY_STEP(3, 0),
  ARM_BOTTOM_STEP(180),
  ARM_TOP_STEP(180),
  HUB_RELAY_STEP(3, 1),
  WAIT_STEP(500),
  ARM_TOP_STEP(100),
  WAIT_STEP(500),
  LIFT_STEP(0, 50),

  TF34_EDGE_FRONT_LIFT_STEP(0.30f, 1800, 0, 15000),
  TF1_FORWARD_BACK_LIFT_STEP(0.30f, 1800, 15000),
  FORWARD_LZ_BACK_STEP(0.50f, 1500, 0.3f, 2000, 15000),

    LIFT_STEP(0, 50),
  TF34_EDGE_FRONT_LIFT_STEP(0.30f, 1800, 0, 15000),
  TF1_FORWARD_BACK_LIFT_STEP(0.30f, 1800, 15000),
  FORWARD_LZ_BACK_STEP(0.50f, 1500, 0.3f, 2000, 15000),

    LIFT_STEP(0, 50),
  TF34_EDGE_FRONT_LIFT_STEP(0.30f, 1800, 0, 15000),
  TF1_FORWARD_BACK_LIFT_STEP(0.30f, 1800, 15000),
  FORWARD_LZ_BACK_STEP(0.50f, 1500, 0.3f, 2000, 15000),

  END_STEP()
};

const MissionStep mission1[] =
{
  RESET_STEP(),
  HUB_ARM_STEP(5),
  HUB_SPIN_STEP(0),
  HUB_RELAY_STEP(4, RELAY4_ON),
  LIFT_STEP(2300, 2400),
  MOVE_STEP(1.4,-1.0,0,15),
  HUB_ARM_STEP(71), 
  FRONT_LIMIT_STEP(0.4f, 15000),
  RESET_STEP_2(),
  LASER5_PICK_VERIFY_STEP(-0.25f, +0.04f, 71, 15000),
  LIFT_STEP(2550, 2650),
  HUB_ARM_STEP(10),
  WAIT_STEP(1000),
  HUB_SPIN_STEP(133),
  WAIT_STEP(1000),
  HUB_WAIT_INPUT_STEP(HUB_LDR1, 1, 0),    
  HUB_RELAY_STEP(4, 1),
  END_STEP()
};

//BLUE
const MissionStep mission2[] =
{
  RESET_STEP(),
  HUB_ARM_STEP(5),
  HUB_SPIN_STEP(0),
  HUB_RELAY_STEP(4, RELAY4_ON),
  LIFT_STEP(2300, 2400),
  MOVE_STEP(1.4,0.5,0,15),
  HUB_ARM_STEP(72),
  FRONT_LIMIT_STEP(0.4f, 15000),
  RESET_STEP_2(),
  LASER5_PICK_VERIFY_STEP(0.25f, +0.04f, 72, 15000),
  LIFT_STEP(2550, 2650),
  HUB_ARM_STEP(10),
  WAIT_STEP(1000),
  HUB_SPIN_STEP(133),
  WAIT_STEP(1000),
  HUB_WAIT_INPUT_STEP(HUB_LDR1, 1, 0),    
  HUB_RELAY_STEP(4, 1),
  END_STEP()
};

/////////////////////////////////////////////////////////////////////////////////MissionStep/////////////////////////////////////////////////////////////////////////////////

// ===========================================================================
//  >>> PATTERN PROGRAMS - one per code the keypad can send. <<<
//
//  ชื่อ array คือ 'p' + รหัส  ส่วนชื่อในทะเบียนคือ "ตัวรหัสเป๊ะ ๆ"
//  master ไม่แกะความหมายของหลักไหนเลย มันแค่ strcmp ชื่อกับรหัสที่รับมา
//
//    Mode 0 : mode(0) + field(0=BLUE,1=RED) + line(1..3) + box(4 หลัก 0/1)
//    Mode 1 : mode(1) + field(0=BLUE,1=RED) + step(0,2,3) + row(1..3) + row
//
//  หลักที่ 5 ของ Mode 1 ขึ้นกับ step:  step 0 = ทำทุก step เลือกได้ 2 แถว
//  (หลัก 4 กับ 5 เป็นแถว ซ้ำแถวเดิมได้ เช่น 11022)  ส่วน step 2/3 เลือกแถวเดียว
//  ที่หลัก 4 แล้วหลัก 5 เป็น 0 ตายตัว
//
//  ทุกอันเป็นโครงเปล่า RESET -> END รอเติมของจริง เติมได้เลยไม่ต้องแก้ที่อื่น
//  ตราบใดที่ยังไม่เปลี่ยนชื่อ  ห้ามแทรกอันใหม่ไว้หน้า exMission/MISSION 1/2
//  ในทะเบียนข้างล่าง เพราะคำสั่ง "Mn" ทาง USB ยังอ้างด้วยลำดับอยู่
// ===========================================================================

// ---------------------------- Mode 0 | Field 0 (BLUE) | Line 1 ----------------------------

const MissionStep p0010000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0010111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_BLUE, MOVE_START_SLIDE_L1_NOLIDAR_BLUE, MOVE_START_YAW_L1_NOLIDAR_BLUE, MOVE_START_SPEED_L1_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

const MissionStep p0011111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_BLUE, MOVE_START_SLIDE_L1_BLUE, MOVE_START_YAW_L1_BLUE, MOVE_START_SPEED_L1_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_BLUE, MOVE_SETUP_SLIDE_L1_BLUE, MOVE_SETUP_YAW_L1_BLUE, MOVE_SETUP_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_BLUE, MOVE_STANDBY_SLIDE_L1_BLUE, MOVE_STANDBY_YAW_L1_BLUE, MOVE_STANDBY_SPEED_L1_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_BLUE, MOVE_RAMP_SLIDE_L1_BLUE, MOVE_RAMP_YAW_L1_BLUE, MOVE_RAMP_SPEED_L1_BLUE),

  END_STEP()
};

// ---------------------------- Mode 0 | Field 0 (BLUE) | Line 2 ----------------------------

const MissionStep p0020000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0020111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_BLUE, MOVE_START_SLIDE_L2_NOLIDAR_BLUE, MOVE_START_YAW_L2_NOLIDAR_BLUE, MOVE_START_SPEED_L2_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

const MissionStep p0021111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_BLUE, MOVE_START_SLIDE_L2_BLUE, MOVE_START_YAW_L2_BLUE, MOVE_START_SPEED_L2_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_BLUE, MOVE_SETUP_SLIDE_L2_BLUE, MOVE_SETUP_YAW_L2_BLUE, MOVE_SETUP_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_BLUE, MOVE_STANDBY_SLIDE_L2_BLUE, MOVE_STANDBY_YAW_L2_BLUE, MOVE_STANDBY_SPEED_L2_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_BLUE, MOVE_RAMP_SLIDE_L2_BLUE, MOVE_RAMP_YAW_L2_BLUE, MOVE_RAMP_SPEED_L2_BLUE),

  END_STEP()
};

// ---------------------------- Mode 0 | Field 0 (BLUE) | Line 3 ----------------------------

const MissionStep p0030000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0030111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_BLUE, MOVE_START_SLIDE_L3_NOLIDAR_BLUE, MOVE_START_YAW_L3_NOLIDAR_BLUE, MOVE_START_SPEED_L3_NOLIDAR_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_BLUE, MOVE_SLIDE_AFTERLIMIT_BLUE, MOVE_YAW_AFTERLIMIT_BLUE, MOVE_SPEED_AFTERLIMIT_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

const MissionStep p0031111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_BLUE, MOVE_START_SLIDE_L3_BLUE, MOVE_START_YAW_L3_BLUE, MOVE_START_SPEED_L3_BLUE),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_BLUE, MOVE_SLIDE_BEFORELIDAR_BLUE, MOVE_YAW_BEFORELIDAR_BLUE, MOVE_SPEED_BEFORELIDAR_BLUE),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_BLUE, MOVE_SETUP_SLIDE_L3_BLUE, MOVE_SETUP_YAW_L3_BLUE, MOVE_SETUP_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_BLUE, MOVE_STANDBY_SLIDE_L3_BLUE, MOVE_STANDBY_YAW_L3_BLUE, MOVE_STANDBY_SPEED_L3_BLUE),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_BLUE, MOVE_RAMP_SLIDE_L3_BLUE, MOVE_RAMP_YAW_L3_BLUE, MOVE_RAMP_SPEED_L3_BLUE),

  END_STEP()
};

// ---------------------------- Mode 0 | Field 1 (RED) | Line 1 ----------------------------

const MissionStep p0110000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),
  
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0110111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L1_NOLIDAR_RED, MOVE_START_SLIDE_L1_NOLIDAR_RED, MOVE_START_YAW_L1_NOLIDAR_RED, MOVE_START_SPEED_L1_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

const MissionStep p0111111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L1_RED, MOVE_START_SLIDE_L1_RED, MOVE_START_YAW_L1_RED, MOVE_START_SPEED_L1_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),
  
  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L1_RED, MOVE_SETUP_SLIDE_L1_RED, MOVE_SETUP_YAW_L1_RED, MOVE_SETUP_SPEED_L1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L1_RED, MOVE_STANDBY_SLIDE_L1_RED, MOVE_STANDBY_YAW_L1_RED, MOVE_STANDBY_SPEED_L1_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L1_RED, MOVE_RAMP_SLIDE_L1_RED, MOVE_RAMP_YAW_L1_RED, MOVE_RAMP_SPEED_L1_RED),

  END_STEP()
};

// ---------------------------- Mode 0 | Field 1 (RED) | Line 2 ----------------------------

const MissionStep p0120000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0120111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L2_NOLIDAR_RED, MOVE_START_SLIDE_L2_NOLIDAR_RED, MOVE_START_YAW_L2_NOLIDAR_RED, MOVE_START_SPEED_L2_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------
  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

const MissionStep p0121111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L2_RED, MOVE_START_SLIDE_L2_RED, MOVE_START_YAW_L2_RED, MOVE_START_SPEED_L2_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_FLOOR_TOF34, FRONT_PULSE_FLOOR_TOF34, BACK_PULSE_FLOOR_TOF34, TIMEOUT_FLOOR_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_FLOOR_TF1, BACK_PULSE_FLOOR_TF1, TIMEOUT_FLOOR_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L2_RED, MOVE_SETUP_SLIDE_L2_RED, MOVE_SETUP_YAW_L2_RED, MOVE_SETUP_SPEED_L2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L2_RED, MOVE_STANDBY_SLIDE_L2_RED, MOVE_STANDBY_YAW_L2_RED, MOVE_STANDBY_SPEED_L2_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L2_RED, MOVE_RAMP_SLIDE_L2_RED, MOVE_RAMP_YAW_L2_RED, MOVE_RAMP_SPEED_L2_RED),

  END_STEP()
};

// ---------------------------- Mode 0 | Field 1 (RED) | Line 3 ----------------------------

const MissionStep p0130000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  
  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0130111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  MOVE_STEP(MOVE_START_FORWARD_L3_NOLIDAR_RED, MOVE_START_SLIDE_L3_NOLIDAR_RED, MOVE_START_YAW_L3_NOLIDAR_RED, MOVE_START_SPEED_L3_NOLIDAR_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131000[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131001[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131010[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131011[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  MOVE_STEP(MOVE_FORWARD_AFTERLIMIT_RED, MOVE_SLIDE_AFTERLIMIT_RED, MOVE_YAW_AFTERLIMIT_RED, MOVE_SPEED_AFTERLIMIT_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131100[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_UP_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131101[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131110[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

const MissionStep p0131111[] =
{
  RESET_STEP(),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  
  MOVE_STEP(MOVE_START_FORWARD_L3_RED, MOVE_START_SLIDE_L3_RED, MOVE_START_YAW_L3_RED, MOVE_START_SPEED_L3_RED),
  
  LIFT_STEP(LIFT_LV1_FRONT, LIFT_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_LV3_FRONT, LIFT_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV3_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),

  //----------------------------------------------------------

  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),
  LIFT_STEP(LIFT_LV2_FRONT, LIFT_LV2_BACK),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_UP_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  ARM_BOTTOM_STEP(BOTTOM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_ZERO, LIFT_LV2_BACK),
  LASER3_FORWARD_STEP(FORWARD_SPEED_LASER3, TIMEOUT_LASER3),
  LIFT_STEP(LIFT_ZERO, LIFT_ZERO),

  LASER4_FORWARD_STEP(FORWARD_SPEED_LASER4, TIMEOUT_LASER4),
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  ARM_TOP_STEP(ARM_FRONT_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_FORWARD_BEFORELIDAR_RED, MOVE_SLIDE_BEFORELIDAR_RED, MOVE_YAW_BEFORELIDAR_RED, MOVE_SPEED_BEFORELIDAR_RED),

  //----------------------------------------------------------

  LIDAR_PARK_STEP(DISTANCE_LIDAR, TIMEOUT_LIDAR),

  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),
  ARM_TOP_STEP(ARM_BACK_DOWN_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_DOWN_FRONT, LIFT_DOWN_BACK),
  TF34_EDGE_FRONT_LIFT_STEP(FORWARD_SPEED_TOF34, FRONT_PULSE_TOF34, BACK_PULSE_TOF34, TIMEOUT_TOF34),
  TF1_FORWARD_BACK_LIFT_STEP(FORWARD_SPEED_TF1, BACK_PULSE_TF1, TIMEOUT_TF1),
  FORWARD_LZ_BACK_STEP(FORWARD_SPEED_LZ_BACK, FORWARDTIME_LZ_BACK, BACK_SPEED_LZ_BACK, BACKTIME_LZ_BACK, TIMEOUT_LZ_BACK),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  FRONT_LIMIT_STEP(FORWARD_SPEED_LIMIT, TIMEOUT_LIMIT),
  RESET_STEP_2(),

  MOVE_STEP(MOVE_SETUP_FORWARD_L3_RED, MOVE_SETUP_SLIDE_L3_RED, MOVE_SETUP_YAW_L3_RED, MOVE_SETUP_SPEED_L3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_L3_RED, MOVE_STANDBY_SLIDE_L3_RED, MOVE_STANDBY_YAW_L3_RED, MOVE_STANDBY_SPEED_L3_RED),
  MOVE_STEP(MOVE_RAMP_FORWARD_L3_RED, MOVE_RAMP_SLIDE_L3_RED, MOVE_RAMP_YAW_L3_RED, MOVE_RAMP_SPEED_L3_RED),

  END_STEP()
};

// ---------------------------- Mode 1 | Field 0 (BLUE) | Step 0 (ALL, two rows) ----------------------------

const MissionStep p10011[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_BLUE, MOVE_SETUP_SLIDE_R1_BLUE, MOVE_SETUP_YAW_R1_BLUE, MOVE_SETUP_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_BLUE, MOVE_STANDBY_SLIDE_R1_BLUE, MOVE_STANDBY_YAW_R1_BLUE, MOVE_STANDBY_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R1_BLUE, MOVE_GO_SLIDE_R1_BLUE, MOVE_GO_YAW_R1_BLUE, MOVE_GO_SPEED_R1_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_BLUE, MOVE_AFTER_GO_SLIDE_R1_BLUE, MOVE_AFTER_GO_YAW_R1_BLUE, MOVE_AFTER_GO_SPEED_R1_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R1_BLUE, MOVE_LV3_SLIDE_R1_BLUE, MOVE_LV3_YAW_R1_BLUE, MOVE_LV3_SPEED_R1_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10012[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_BLUE, MOVE_SETUP_SLIDE_R1_BLUE, MOVE_SETUP_YAW_R1_BLUE, MOVE_SETUP_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_BLUE, MOVE_STANDBY_SLIDE_R1_BLUE, MOVE_STANDBY_YAW_R1_BLUE, MOVE_STANDBY_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R1_BLUE, MOVE_GO_SLIDE_R1_BLUE, MOVE_GO_YAW_R1_BLUE, MOVE_GO_SPEED_R1_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_BLUE, MOVE_AFTER_GO_SLIDE_R1_BLUE, MOVE_AFTER_GO_YAW_R1_BLUE, MOVE_AFTER_GO_SPEED_R1_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R2_BLUE, MOVE_LV3_SLIDE_R2_BLUE, MOVE_LV3_YAW_R2_BLUE, MOVE_LV3_SPEED_R2_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10013[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_BLUE, MOVE_SETUP_SLIDE_R1_BLUE, MOVE_SETUP_YAW_R1_BLUE, MOVE_SETUP_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_BLUE, MOVE_STANDBY_SLIDE_R1_BLUE, MOVE_STANDBY_YAW_R1_BLUE, MOVE_STANDBY_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R1_BLUE, MOVE_GO_SLIDE_R1_BLUE, MOVE_GO_YAW_R1_BLUE, MOVE_GO_SPEED_R1_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_BLUE, MOVE_AFTER_GO_SLIDE_R1_BLUE, MOVE_AFTER_GO_YAW_R1_BLUE, MOVE_AFTER_GO_SPEED_R1_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R3_BLUE, MOVE_LV3_SLIDE_R3_BLUE, MOVE_LV3_YAW_R3_BLUE, MOVE_LV3_SPEED_R3_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10021[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_BLUE, MOVE_SETUP_SLIDE_R2_BLUE, MOVE_SETUP_YAW_R2_BLUE, MOVE_SETUP_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_BLUE, MOVE_STANDBY_SLIDE_R2_BLUE, MOVE_STANDBY_YAW_R2_BLUE, MOVE_STANDBY_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R2_BLUE, MOVE_GO_SLIDE_R2_BLUE, MOVE_GO_YAW_R2_BLUE, MOVE_GO_SPEED_R2_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_BLUE, MOVE_AFTER_GO_SLIDE_R2_BLUE, MOVE_AFTER_GO_YAW_R2_BLUE, MOVE_AFTER_GO_SPEED_R2_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R1_BLUE, MOVE_LV3_SLIDE_R1_BLUE, MOVE_LV3_YAW_R1_BLUE, MOVE_LV3_SPEED_R1_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10022[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_BLUE, MOVE_SETUP_SLIDE_R2_BLUE, MOVE_SETUP_YAW_R2_BLUE, MOVE_SETUP_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_BLUE, MOVE_STANDBY_SLIDE_R2_BLUE, MOVE_STANDBY_YAW_R2_BLUE, MOVE_STANDBY_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R2_BLUE, MOVE_GO_SLIDE_R2_BLUE, MOVE_GO_YAW_R2_BLUE, MOVE_GO_SPEED_R2_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_BLUE, MOVE_AFTER_GO_SLIDE_R2_BLUE, MOVE_AFTER_GO_YAW_R2_BLUE, MOVE_AFTER_GO_SPEED_R2_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R2_BLUE, MOVE_LV3_SLIDE_R2_BLUE, MOVE_LV3_YAW_R2_BLUE, MOVE_LV3_SPEED_R2_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10023[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_BLUE, MOVE_SETUP_SLIDE_R2_BLUE, MOVE_SETUP_YAW_R2_BLUE, MOVE_SETUP_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_BLUE, MOVE_STANDBY_SLIDE_R2_BLUE, MOVE_STANDBY_YAW_R2_BLUE, MOVE_STANDBY_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R2_BLUE, MOVE_GO_SLIDE_R2_BLUE, MOVE_GO_YAW_R2_BLUE, MOVE_GO_SPEED_R2_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_BLUE, MOVE_AFTER_GO_SLIDE_R2_BLUE, MOVE_AFTER_GO_YAW_R2_BLUE, MOVE_AFTER_GO_SPEED_R2_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R3_BLUE, MOVE_LV3_SLIDE_R3_BLUE, MOVE_LV3_YAW_R3_BLUE, MOVE_LV3_SPEED_R3_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10031[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_BLUE, MOVE_SETUP_SLIDE_R3_BLUE, MOVE_SETUP_YAW_R3_BLUE, MOVE_SETUP_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_BLUE, MOVE_STANDBY_SLIDE_R3_BLUE, MOVE_STANDBY_YAW_R3_BLUE, MOVE_STANDBY_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R3_BLUE, MOVE_GO_SLIDE_R3_BLUE, MOVE_GO_YAW_R3_BLUE, MOVE_GO_SPEED_R3_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_BLUE, MOVE_AFTER_GO_SLIDE_R3_BLUE, MOVE_AFTER_GO_YAW_R3_BLUE, MOVE_AFTER_GO_SPEED_R3_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R1_BLUE, MOVE_LV3_SLIDE_R1_BLUE, MOVE_LV3_YAW_R1_BLUE, MOVE_LV3_SPEED_R1_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10032[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_BLUE, MOVE_SETUP_SLIDE_R3_BLUE, MOVE_SETUP_YAW_R3_BLUE, MOVE_SETUP_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_BLUE, MOVE_STANDBY_SLIDE_R3_BLUE, MOVE_STANDBY_YAW_R3_BLUE, MOVE_STANDBY_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R3_BLUE, MOVE_GO_SLIDE_R3_BLUE, MOVE_GO_YAW_R3_BLUE, MOVE_GO_SPEED_R3_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_BLUE, MOVE_AFTER_GO_SLIDE_R3_BLUE, MOVE_AFTER_GO_YAW_R3_BLUE, MOVE_AFTER_GO_SPEED_R3_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R2_BLUE, MOVE_LV3_SLIDE_R2_BLUE, MOVE_LV3_YAW_R2_BLUE, MOVE_LV3_SPEED_R2_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p10033[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_BLUE, MOVE_SETUP_SLIDE_R3_BLUE, MOVE_SETUP_YAW_R3_BLUE, MOVE_SETUP_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_BLUE, MOVE_STANDBY_SLIDE_R3_BLUE, MOVE_STANDBY_YAW_R3_BLUE, MOVE_STANDBY_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R3_BLUE, MOVE_GO_SLIDE_R3_BLUE, MOVE_GO_YAW_R3_BLUE, MOVE_GO_SPEED_R3_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_BLUE, MOVE_AFTER_GO_SLIDE_R3_BLUE, MOVE_AFTER_GO_YAW_R3_BLUE, MOVE_AFTER_GO_SPEED_R3_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R3_BLUE, MOVE_LV3_SLIDE_R3_BLUE, MOVE_LV3_YAW_R3_BLUE, MOVE_LV3_SPEED_R3_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

// ---------------------------- Mode 1 | Field 0 (BLUE) | Step 2 (one row) ----------------------------

const MissionStep p10210[] =
{
  RESET_STEP(),
  
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_BLUE, MOVE_SETUP_SLIDE_R1_BLUE, MOVE_SETUP_YAW_R1_BLUE, MOVE_SETUP_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_BLUE, MOVE_STANDBY_SLIDE_R1_BLUE, MOVE_STANDBY_YAW_R1_BLUE, MOVE_STANDBY_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R1_BLUE, MOVE_GO_SLIDE_R1_BLUE, MOVE_GO_YAW_R1_BLUE, MOVE_GO_SPEED_R1_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_BLUE, MOVE_AFTER_GO_SLIDE_R1_BLUE, MOVE_AFTER_GO_YAW_R1_BLUE, MOVE_AFTER_GO_SPEED_R1_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

const MissionStep p10220[] =
{
  RESET_STEP(),
  
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_BLUE, MOVE_SETUP_SLIDE_R2_BLUE, MOVE_SETUP_YAW_R2_BLUE, MOVE_SETUP_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_BLUE, MOVE_STANDBY_SLIDE_R2_BLUE, MOVE_STANDBY_YAW_R2_BLUE, MOVE_STANDBY_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R2_BLUE, MOVE_GO_SLIDE_R2_BLUE, MOVE_GO_YAW_R2_BLUE, MOVE_GO_SPEED_R2_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_BLUE, MOVE_AFTER_GO_SLIDE_R2_BLUE, MOVE_AFTER_GO_YAW_R2_BLUE, MOVE_AFTER_GO_SPEED_R2_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  
  END_STEP()
};

const MissionStep p10230[] =
{
  RESET_STEP(),
  
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_BLUE, MOVE_SETUP_SLIDE_R3_BLUE, MOVE_SETUP_YAW_R3_BLUE, MOVE_SETUP_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_BLUE, MOVE_STANDBY_SLIDE_R3_BLUE, MOVE_STANDBY_YAW_R3_BLUE, MOVE_STANDBY_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_GO_FORWARD_R3_BLUE, MOVE_GO_SLIDE_R3_BLUE, MOVE_GO_YAW_R3_BLUE, MOVE_GO_SPEED_R3_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_BLUE, MOVE_AFTER_GO_SLIDE_R3_BLUE, MOVE_AFTER_GO_YAW_R3_BLUE, MOVE_AFTER_GO_SPEED_R3_BLUE),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  
  END_STEP()
};

// ---------------------------- Mode 1 | Field 0 (BLUE) | Step 3 (one row) ----------------------------

const MissionStep p10310[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_BLUE, MOVE_SETUP_SLIDE_R1_BLUE, MOVE_SETUP_YAW_R1_BLUE, MOVE_SETUP_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_BLUE, MOVE_STANDBY_SLIDE_R1_BLUE, MOVE_STANDBY_YAW_R1_BLUE, MOVE_STANDBY_SPEED_R1_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_BLUE, MOVE_BACKWARD_SLIDE_R1_BLUE, MOVE_BACKWARD_YAW_R1_BLUE, MOVE_BACKWARD_SPEED_R1_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_FRONT_LV3_DEG),
  MOVE_STEP(MOVE_LV3_FORWARD_R1_BLUE, MOVE_LV3_SLIDE_R1_BLUE, MOVE_LV3_YAW_R1_BLUE, MOVE_LV3_SPEED_R1_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV3_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),

  ARM_TOP_STEP(ARM_PUSH_FRONT_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

const MissionStep p10320[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_BLUE, MOVE_SETUP_SLIDE_R2_BLUE, MOVE_SETUP_YAW_R2_BLUE, MOVE_SETUP_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_BLUE, MOVE_STANDBY_SLIDE_R2_BLUE, MOVE_STANDBY_YAW_R2_BLUE, MOVE_STANDBY_SPEED_R2_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_BLUE, MOVE_BACKWARD_SLIDE_R2_BLUE, MOVE_BACKWARD_YAW_R2_BLUE, MOVE_BACKWARD_SPEED_R2_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_FRONT_LV3_DEG),
  MOVE_STEP(MOVE_LV3_FORWARD_R2_BLUE, MOVE_LV3_SLIDE_R2_BLUE, MOVE_LV3_YAW_R2_BLUE, MOVE_LV3_SPEED_R2_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV3_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),

  ARM_TOP_STEP(ARM_PUSH_FRONT_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

const MissionStep p10330[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_BLUE, MOVE_SETUP_SLIDE_R3_BLUE, MOVE_SETUP_YAW_R3_BLUE, MOVE_SETUP_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_BLUE, MOVE_STANDBY_SLIDE_R3_BLUE, MOVE_STANDBY_YAW_R3_BLUE, MOVE_STANDBY_SPEED_R3_BLUE),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_BLUE, MOVE_BACKWARD_SLIDE_R3_BLUE, MOVE_BACKWARD_YAW_R3_BLUE, MOVE_BACKWARD_SPEED_R3_BLUE),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_FRONT_LV3_DEG),
  MOVE_STEP(MOVE_LV3_FORWARD_R3_BLUE, MOVE_LV3_SLIDE_R3_BLUE, MOVE_LV3_YAW_R3_BLUE, MOVE_LV3_SPEED_R3_BLUE),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV3_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),

  ARM_TOP_STEP(ARM_PUSH_FRONT_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

// ---------------------------- Mode 1 | Field 1 (RED) | Step 0 (ALL, two rows) ----------------------------

const MissionStep p11011[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_RED, MOVE_SETUP_SLIDE_R1_RED, MOVE_SETUP_YAW_R1_RED, MOVE_SETUP_SPEED_R1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_RED, MOVE_STANDBY_SLIDE_R1_RED, MOVE_STANDBY_YAW_R1_RED, MOVE_STANDBY_SPEED_R1_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R1_RED, MOVE_GO_SLIDE_R1_RED, MOVE_GO_YAW_R1_RED, MOVE_GO_SPEED_R1_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_RED, MOVE_AFTER_GO_SLIDE_R1_RED, MOVE_AFTER_GO_YAW_R1_RED, MOVE_AFTER_GO_SPEED_R1_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R1_RED, MOVE_LV3_SLIDE_R1_RED, MOVE_LV3_YAW_R1_RED, MOVE_LV3_SPEED_R1_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11012[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_RED, MOVE_SETUP_SLIDE_R1_RED, MOVE_SETUP_YAW_R1_RED, MOVE_SETUP_SPEED_R1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_RED, MOVE_STANDBY_SLIDE_R1_RED, MOVE_STANDBY_YAW_R1_RED, MOVE_STANDBY_SPEED_R1_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R1_RED, MOVE_GO_SLIDE_R1_RED, MOVE_GO_YAW_R1_RED, MOVE_GO_SPEED_R1_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_RED, MOVE_AFTER_GO_SLIDE_R1_RED, MOVE_AFTER_GO_YAW_R1_RED, MOVE_AFTER_GO_SPEED_R1_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R2_RED, MOVE_LV3_SLIDE_R2_RED, MOVE_LV3_YAW_R2_RED, MOVE_LV3_SPEED_R2_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11013[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_RED, MOVE_SETUP_SLIDE_R1_RED, MOVE_SETUP_YAW_R1_RED, MOVE_SETUP_SPEED_R1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_RED, MOVE_STANDBY_SLIDE_R1_RED, MOVE_STANDBY_YAW_R1_RED, MOVE_STANDBY_SPEED_R1_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R1_RED, MOVE_GO_SLIDE_R1_RED, MOVE_GO_YAW_R1_RED, MOVE_GO_SPEED_R1_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_RED, MOVE_AFTER_GO_SLIDE_R1_RED, MOVE_AFTER_GO_YAW_R1_RED, MOVE_AFTER_GO_SPEED_R1_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R3_RED, MOVE_LV3_SLIDE_R3_RED, MOVE_LV3_YAW_R3_RED, MOVE_LV3_SPEED_R3_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11021[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_RED, MOVE_SETUP_SLIDE_R2_RED, MOVE_SETUP_YAW_R2_RED, MOVE_SETUP_SPEED_R2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_RED, MOVE_STANDBY_SLIDE_R2_RED, MOVE_STANDBY_YAW_R2_RED, MOVE_STANDBY_SPEED_R2_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R2_RED, MOVE_GO_SLIDE_R2_RED, MOVE_GO_YAW_R2_RED, MOVE_GO_SPEED_R2_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_RED, MOVE_AFTER_GO_SLIDE_R2_RED, MOVE_AFTER_GO_YAW_R2_RED, MOVE_AFTER_GO_SPEED_R2_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R1_RED, MOVE_LV3_SLIDE_R1_RED, MOVE_LV3_YAW_R1_RED, MOVE_LV3_SPEED_R1_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11022[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_RED, MOVE_SETUP_SLIDE_R2_RED, MOVE_SETUP_YAW_R2_RED, MOVE_SETUP_SPEED_R2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_RED, MOVE_STANDBY_SLIDE_R2_RED, MOVE_STANDBY_YAW_R2_RED, MOVE_STANDBY_SPEED_R2_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R2_RED, MOVE_GO_SLIDE_R2_RED, MOVE_GO_YAW_R2_RED, MOVE_GO_SPEED_R2_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_RED, MOVE_AFTER_GO_SLIDE_R2_RED, MOVE_AFTER_GO_YAW_R2_RED, MOVE_AFTER_GO_SPEED_R2_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R2_RED, MOVE_LV3_SLIDE_R2_RED, MOVE_LV3_YAW_R2_RED, MOVE_LV3_SPEED_R2_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11023[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_RED, MOVE_SETUP_SLIDE_R2_RED, MOVE_SETUP_YAW_R2_RED, MOVE_SETUP_SPEED_R2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_RED, MOVE_STANDBY_SLIDE_R2_RED, MOVE_STANDBY_YAW_R2_RED, MOVE_STANDBY_SPEED_R2_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R2_RED, MOVE_GO_SLIDE_R2_RED, MOVE_GO_YAW_R2_RED, MOVE_GO_SPEED_R2_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_RED, MOVE_AFTER_GO_SLIDE_R2_RED, MOVE_AFTER_GO_YAW_R2_RED, MOVE_AFTER_GO_SPEED_R2_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R3_RED, MOVE_LV3_SLIDE_R3_RED, MOVE_LV3_YAW_R3_RED, MOVE_LV3_SPEED_R3_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11031[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_RED, MOVE_SETUP_SLIDE_R3_RED, MOVE_SETUP_YAW_R3_RED, MOVE_SETUP_SPEED_R3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_RED, MOVE_STANDBY_SLIDE_R3_RED, MOVE_STANDBY_YAW_R3_RED, MOVE_STANDBY_SPEED_R3_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R3_RED, MOVE_GO_SLIDE_R3_RED, MOVE_GO_YAW_R3_RED, MOVE_GO_SPEED_R3_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_RED, MOVE_AFTER_GO_SLIDE_R3_RED, MOVE_AFTER_GO_YAW_R3_RED, MOVE_AFTER_GO_SPEED_R3_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R1_RED, MOVE_LV3_SLIDE_R1_RED, MOVE_LV3_YAW_R1_RED, MOVE_LV3_SPEED_R1_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11032[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_RED, MOVE_SETUP_SLIDE_R3_RED, MOVE_SETUP_YAW_R3_RED, MOVE_SETUP_SPEED_R3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_RED, MOVE_STANDBY_SLIDE_R3_RED, MOVE_STANDBY_YAW_R3_RED, MOVE_STANDBY_SPEED_R3_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R3_RED, MOVE_GO_SLIDE_R3_RED, MOVE_GO_YAW_R3_RED, MOVE_GO_SPEED_R3_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_RED, MOVE_AFTER_GO_SLIDE_R3_RED, MOVE_AFTER_GO_YAW_R3_RED, MOVE_AFTER_GO_SPEED_R3_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R2_RED, MOVE_LV3_SLIDE_R2_RED, MOVE_LV3_YAW_R2_RED, MOVE_LV3_SPEED_R2_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

const MissionStep p11033[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_RED, MOVE_SETUP_SLIDE_R3_RED, MOVE_SETUP_YAW_R3_RED, MOVE_SETUP_SPEED_R3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_RED, MOVE_STANDBY_SLIDE_R3_RED, MOVE_STANDBY_YAW_R3_RED, MOVE_STANDBY_SPEED_R3_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R3_RED, MOVE_GO_SLIDE_R3_RED, MOVE_GO_YAW_R3_RED, MOVE_GO_SPEED_R3_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_RED, MOVE_AFTER_GO_SLIDE_R3_RED, MOVE_AFTER_GO_YAW_R3_RED, MOVE_AFTER_GO_SPEED_R3_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_BACK_LV3_DEG),
  ARM_BOTTOM_STEP(BOTTOM_BACK_DEG),

  MOVE_STEP(MOVE_LV3_FORWARD_R3_RED, MOVE_LV3_SLIDE_R3_RED, MOVE_LV3_YAW_R3_RED, MOVE_LV3_SPEED_R3_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_BACK_LV3_DEG),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_ON),

  ARM_TOP_STEP(ARM_PUSH_BACK_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),

  END_STEP()
};

// ---------------------------- Mode 1 | Field 1 (RED) | Step 2 (one row) ----------------------------

const MissionStep p11210[] =
{
  RESET_STEP(),
  
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_RED, MOVE_SETUP_SLIDE_R1_RED, MOVE_SETUP_YAW_R1_RED, MOVE_SETUP_SPEED_R1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_RED, MOVE_STANDBY_SLIDE_R1_RED, MOVE_STANDBY_YAW_R1_RED, MOVE_STANDBY_SPEED_R1_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R1_RED, MOVE_GO_SLIDE_R1_RED, MOVE_GO_YAW_R1_RED, MOVE_GO_SPEED_R1_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R1_RED, MOVE_AFTER_GO_SLIDE_R1_RED, MOVE_AFTER_GO_YAW_R1_RED, MOVE_AFTER_GO_SPEED_R1_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

const MissionStep p11220[] =
{
  RESET_STEP(),
  
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_RED, MOVE_SETUP_SLIDE_R2_RED, MOVE_SETUP_YAW_R2_RED, MOVE_SETUP_SPEED_R2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_RED, MOVE_STANDBY_SLIDE_R2_RED, MOVE_STANDBY_YAW_R2_RED, MOVE_STANDBY_SPEED_R2_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R2_RED, MOVE_GO_SLIDE_R2_RED, MOVE_GO_YAW_R2_RED, MOVE_GO_SPEED_R2_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R2_RED, MOVE_AFTER_GO_SLIDE_R2_RED, MOVE_AFTER_GO_YAW_R2_RED, MOVE_AFTER_GO_SPEED_R2_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  
  END_STEP()
};

const MissionStep p11230[] =
{
  RESET_STEP(),
  
  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_RED, MOVE_SETUP_SLIDE_R3_RED, MOVE_SETUP_YAW_R3_RED, MOVE_SETUP_SPEED_R3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_RED, MOVE_STANDBY_SLIDE_R3_RED, MOVE_STANDBY_YAW_R3_RED, MOVE_STANDBY_SPEED_R3_RED),
  MOVE_STEP(MOVE_GO_FORWARD_R3_RED, MOVE_GO_SLIDE_R3_RED, MOVE_GO_YAW_R3_RED, MOVE_GO_SPEED_R3_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV2_DEG),
  MOVE_STEP(MOVE_AFTER_GO_FORWARD_R3_RED, MOVE_AFTER_GO_SLIDE_R3_RED, MOVE_AFTER_GO_YAW_R3_RED, MOVE_AFTER_GO_SPEED_R3_RED),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),
  ARM_TOP_STEP(ARM_PUSH_FRONT_LV2_DEG),

  //----------------------------------------------------------

  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),
  
  END_STEP()
};

// ---------------------------- Mode 1 | Field 1 (RED) | Step 3 (one row) ----------------------------

const MissionStep p11310[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R1_RED, MOVE_SETUP_SLIDE_R1_RED, MOVE_SETUP_YAW_R1_RED, MOVE_SETUP_SPEED_R1_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R1_RED, MOVE_STANDBY_SLIDE_R1_RED, MOVE_STANDBY_YAW_R1_RED, MOVE_STANDBY_SPEED_R1_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R1_RED, MOVE_BACKWARD_SLIDE_R1_RED, MOVE_BACKWARD_YAW_R1_RED, MOVE_BACKWARD_SPEED_R1_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_FRONT_LV3_DEG),
  MOVE_STEP(MOVE_LV3_FORWARD_R1_RED, MOVE_LV3_SLIDE_R1_RED, MOVE_LV3_YAW_R1_RED, MOVE_LV3_SPEED_R1_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV3_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),

  ARM_TOP_STEP(ARM_PUSH_FRONT_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

const MissionStep p11320[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R2_RED, MOVE_SETUP_SLIDE_R2_RED, MOVE_SETUP_YAW_R2_RED, MOVE_SETUP_SPEED_R2_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R2_RED, MOVE_STANDBY_SLIDE_R2_RED, MOVE_STANDBY_YAW_R2_RED, MOVE_STANDBY_SPEED_R2_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R2_RED, MOVE_BACKWARD_SLIDE_R2_RED, MOVE_BACKWARD_YAW_R2_RED, MOVE_BACKWARD_SPEED_R2_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_FRONT_LV3_DEG),
  MOVE_STEP(MOVE_LV3_FORWARD_R2_RED, MOVE_LV3_SLIDE_R2_RED, MOVE_LV3_YAW_R2_RED, MOVE_LV3_SPEED_R2_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV3_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),

  ARM_TOP_STEP(ARM_PUSH_FRONT_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};

const MissionStep p11330[] =
{
  RESET_STEP(),

  ARM_BOTTOM_STEP(BOTTOM_FRONT_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_OFF),
  HUB_RELAY_STEP(RELAY_BACK, RELAY3_OFF),
  WAIT_STEP(WAIT_STEP_MS),
  HUB_RELAY_STEP(RELAY_ARM, RELAY1_ON),
  ARM_TOP_STEP(ARM_WALK_FRONT_DEG),

  //----------------------------------------------------------
  
  LIFT_STEP(LIFT_START_FRONT, LIFT_START_BACK),
  MOVE_STEP(MOVE_SETUP_FORWARD_R3_RED, MOVE_SETUP_SLIDE_R3_RED, MOVE_SETUP_YAW_R3_RED, MOVE_SETUP_SPEED_R3_RED),
  MOVE_STEP(MOVE_STANDBY_FORWARD_R3_RED, MOVE_STANDBY_SLIDE_R3_RED, MOVE_STANDBY_YAW_R3_RED, MOVE_STANDBY_SPEED_R3_RED),
  MOVE_STEP(MOVE_BACKWARD_FORWARD_R3_RED, MOVE_BACKWARD_SLIDE_R3_RED, MOVE_BACKWARD_YAW_R3_RED, MOVE_BACKWARD_SPEED_R3_RED),

  //----------------------------------------------------------

  LIFT_STEP(LIFT_BOX_LV1_FRONT, LIFT_BOX_LV1_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV2_FRONT, LIFT_BOX_LV2_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),
  LIFT_STEP(LIFT_BOX_LV3_FRONT, LIFT_BOX_LV3_BACK),
  WAIT_STEP(WAIT_LIFT_LEVEL_MS),

  //----------------------------------------------------------

  ARM_TOP_STEP(ARM_WAIT_PLACE_FRONT_LV3_DEG),
  MOVE_STEP(MOVE_LV3_FORWARD_R3_RED, MOVE_LV3_SLIDE_R3_RED, MOVE_LV3_YAW_R3_RED, MOVE_LV3_SPEED_R3_RED),

  //----------------------------------------------------------

  HUB_WAIT_INPUT_STEP(HUB_LDR2, 1, 0),

  ARM_TOP_STEP(ARM_PLACE_FRONT_LV3_DEG),
  HUB_RELAY_STEP(RELAY_FRONT, RELAY2_ON),

  ARM_TOP_STEP(ARM_PUSH_FRONT_LV3_DEG),
  ARM_TOP_STEP(ARM_MIDDLE_DEG),

  END_STEP()
};


const MissionProgram missionPrograms[] =
{
  // ---- ของเดิม อย่าย้ายลำดับ 3 อันนี้ ----
  { "exMission", exMission, ARRAY_COUNT(exMission) },
  { "MISSION 1", mission1, ARRAY_COUNT(mission1) },
  { "MISSION 2", mission2, ARRAY_COUNT(mission2) },

  // ---- Mode 0 | Field 0 (BLUE) | Line 1 ----
  { "0010000", p0010000, ARRAY_COUNT(p0010000) },
  { "0010001", p0010001, ARRAY_COUNT(p0010001) },
  { "0010010", p0010010, ARRAY_COUNT(p0010010) },
  { "0010011", p0010011, ARRAY_COUNT(p0010011) },
  { "0010100", p0010100, ARRAY_COUNT(p0010100) },
  { "0010101", p0010101, ARRAY_COUNT(p0010101) },
  { "0010110", p0010110, ARRAY_COUNT(p0010110) },
  { "0010111", p0010111, ARRAY_COUNT(p0010111) },
  { "0011000", p0011000, ARRAY_COUNT(p0011000) },
  { "0011001", p0011001, ARRAY_COUNT(p0011001) },
  { "0011010", p0011010, ARRAY_COUNT(p0011010) },
  { "0011011", p0011011, ARRAY_COUNT(p0011011) },
  { "0011100", p0011100, ARRAY_COUNT(p0011100) },
  { "0011101", p0011101, ARRAY_COUNT(p0011101) },
  { "0011110", p0011110, ARRAY_COUNT(p0011110) },
  { "0011111", p0011111, ARRAY_COUNT(p0011111) },

  // ---- Mode 0 | Field 0 (BLUE) | Line 2 ----
  { "0020000", p0020000, ARRAY_COUNT(p0020000) },
  { "0020001", p0020001, ARRAY_COUNT(p0020001) },
  { "0020010", p0020010, ARRAY_COUNT(p0020010) },
  { "0020011", p0020011, ARRAY_COUNT(p0020011) },
  { "0020100", p0020100, ARRAY_COUNT(p0020100) },
  { "0020101", p0020101, ARRAY_COUNT(p0020101) },
  { "0020110", p0020110, ARRAY_COUNT(p0020110) },
  { "0020111", p0020111, ARRAY_COUNT(p0020111) },
  { "0021000", p0021000, ARRAY_COUNT(p0021000) },
  { "0021001", p0021001, ARRAY_COUNT(p0021001) },
  { "0021010", p0021010, ARRAY_COUNT(p0021010) },
  { "0021011", p0021011, ARRAY_COUNT(p0021011) },
  { "0021100", p0021100, ARRAY_COUNT(p0021100) },
  { "0021101", p0021101, ARRAY_COUNT(p0021101) },
  { "0021110", p0021110, ARRAY_COUNT(p0021110) },
  { "0021111", p0021111, ARRAY_COUNT(p0021111) },

  // ---- Mode 0 | Field 0 (BLUE) | Line 3 ----
  { "0030000", p0030000, ARRAY_COUNT(p0030000) },
  { "0030001", p0030001, ARRAY_COUNT(p0030001) },
  { "0030010", p0030010, ARRAY_COUNT(p0030010) },
  { "0030011", p0030011, ARRAY_COUNT(p0030011) },
  { "0030100", p0030100, ARRAY_COUNT(p0030100) },
  { "0030101", p0030101, ARRAY_COUNT(p0030101) },
  { "0030110", p0030110, ARRAY_COUNT(p0030110) },
  { "0030111", p0030111, ARRAY_COUNT(p0030111) },
  { "0031000", p0031000, ARRAY_COUNT(p0031000) },
  { "0031001", p0031001, ARRAY_COUNT(p0031001) },
  { "0031010", p0031010, ARRAY_COUNT(p0031010) },
  { "0031011", p0031011, ARRAY_COUNT(p0031011) },
  { "0031100", p0031100, ARRAY_COUNT(p0031100) },
  { "0031101", p0031101, ARRAY_COUNT(p0031101) },
  { "0031110", p0031110, ARRAY_COUNT(p0031110) },
  { "0031111", p0031111, ARRAY_COUNT(p0031111) },

  // ---- Mode 0 | Field 1 (RED) | Line 1 ----
  { "0110000", p0110000, ARRAY_COUNT(p0110000) },
  { "0110001", p0110001, ARRAY_COUNT(p0110001) },
  { "0110010", p0110010, ARRAY_COUNT(p0110010) },
  { "0110011", p0110011, ARRAY_COUNT(p0110011) },
  { "0110100", p0110100, ARRAY_COUNT(p0110100) },
  { "0110101", p0110101, ARRAY_COUNT(p0110101) },
  { "0110110", p0110110, ARRAY_COUNT(p0110110) },
  { "0110111", p0110111, ARRAY_COUNT(p0110111) },
  { "0111000", p0111000, ARRAY_COUNT(p0111000) },
  { "0111001", p0111001, ARRAY_COUNT(p0111001) },
  { "0111010", p0111010, ARRAY_COUNT(p0111010) },
  { "0111011", p0111011, ARRAY_COUNT(p0111011) },
  { "0111100", p0111100, ARRAY_COUNT(p0111100) },
  { "0111101", p0111101, ARRAY_COUNT(p0111101) },
  { "0111110", p0111110, ARRAY_COUNT(p0111110) },
  { "0111111", p0111111, ARRAY_COUNT(p0111111) },

  // ---- Mode 0 | Field 1 (RED) | Line 2 ----
  { "0120000", p0120000, ARRAY_COUNT(p0120000) },
  { "0120001", p0120001, ARRAY_COUNT(p0120001) },
  { "0120010", p0120010, ARRAY_COUNT(p0120010) },
  { "0120011", p0120011, ARRAY_COUNT(p0120011) },
  { "0120100", p0120100, ARRAY_COUNT(p0120100) },
  { "0120101", p0120101, ARRAY_COUNT(p0120101) },
  { "0120110", p0120110, ARRAY_COUNT(p0120110) },
  { "0120111", p0120111, ARRAY_COUNT(p0120111) },
  { "0121000", p0121000, ARRAY_COUNT(p0121000) },
  { "0121001", p0121001, ARRAY_COUNT(p0121001) },
  { "0121010", p0121010, ARRAY_COUNT(p0121010) },
  { "0121011", p0121011, ARRAY_COUNT(p0121011) },
  { "0121100", p0121100, ARRAY_COUNT(p0121100) },
  { "0121101", p0121101, ARRAY_COUNT(p0121101) },
  { "0121110", p0121110, ARRAY_COUNT(p0121110) },
  { "0121111", p0121111, ARRAY_COUNT(p0121111) },

  // ---- Mode 0 | Field 1 (RED) | Line 3 ----
  { "0130000", p0130000, ARRAY_COUNT(p0130000) },
  { "0130001", p0130001, ARRAY_COUNT(p0130001) },
  { "0130010", p0130010, ARRAY_COUNT(p0130010) },
  { "0130011", p0130011, ARRAY_COUNT(p0130011) },
  { "0130100", p0130100, ARRAY_COUNT(p0130100) },
  { "0130101", p0130101, ARRAY_COUNT(p0130101) },
  { "0130110", p0130110, ARRAY_COUNT(p0130110) },
  { "0130111", p0130111, ARRAY_COUNT(p0130111) },
  { "0131000", p0131000, ARRAY_COUNT(p0131000) },
  { "0131001", p0131001, ARRAY_COUNT(p0131001) },
  { "0131010", p0131010, ARRAY_COUNT(p0131010) },
  { "0131011", p0131011, ARRAY_COUNT(p0131011) },
  { "0131100", p0131100, ARRAY_COUNT(p0131100) },
  { "0131101", p0131101, ARRAY_COUNT(p0131101) },
  { "0131110", p0131110, ARRAY_COUNT(p0131110) },
  { "0131111", p0131111, ARRAY_COUNT(p0131111) },

  // ---- Mode 1 | Field 0 (BLUE) | Step 0 (ALL, two rows) ----
  { "10011", p10011, ARRAY_COUNT(p10011) },
  { "10012", p10012, ARRAY_COUNT(p10012) },
  { "10013", p10013, ARRAY_COUNT(p10013) },
  { "10021", p10021, ARRAY_COUNT(p10021) },
  { "10022", p10022, ARRAY_COUNT(p10022) },
  { "10023", p10023, ARRAY_COUNT(p10023) },
  { "10031", p10031, ARRAY_COUNT(p10031) },
  { "10032", p10032, ARRAY_COUNT(p10032) },
  { "10033", p10033, ARRAY_COUNT(p10033) },

  // ---- Mode 1 | Field 0 (BLUE) | Step 2 (one row) ----
  { "10210", p10210, ARRAY_COUNT(p10210) },
  { "10220", p10220, ARRAY_COUNT(p10220) },
  { "10230", p10230, ARRAY_COUNT(p10230) },

  // ---- Mode 1 | Field 0 (BLUE) | Step 3 (one row) ----
  { "10310", p10310, ARRAY_COUNT(p10310) },
  { "10320", p10320, ARRAY_COUNT(p10320) },
  { "10330", p10330, ARRAY_COUNT(p10330) },

  // ---- Mode 1 | Field 1 (RED) | Step 0 (ALL, two rows) ----
  { "11011", p11011, ARRAY_COUNT(p11011) },
  { "11012", p11012, ARRAY_COUNT(p11012) },
  { "11013", p11013, ARRAY_COUNT(p11013) },
  { "11021", p11021, ARRAY_COUNT(p11021) },
  { "11022", p11022, ARRAY_COUNT(p11022) },
  { "11023", p11023, ARRAY_COUNT(p11023) },
  { "11031", p11031, ARRAY_COUNT(p11031) },
  { "11032", p11032, ARRAY_COUNT(p11032) },
  { "11033", p11033, ARRAY_COUNT(p11033) },

  // ---- Mode 1 | Field 1 (RED) | Step 2 (one row) ----
  { "11210", p11210, ARRAY_COUNT(p11210) },
  { "11220", p11220, ARRAY_COUNT(p11220) },
  { "11230", p11230, ARRAY_COUNT(p11230) },

  // ---- Mode 1 | Field 1 (RED) | Step 3 (one row) ----
  { "11310", p11310, ARRAY_COUNT(p11310) },
  { "11320", p11320, ARRAY_COUNT(p11320) },
  { "11330", p11330, ARRAY_COUNT(p11330) },
};

const size_t MISSION_PROGRAM_COUNT = ARRAY_COUNT(missionPrograms);

uint8_t selectedMission = 0;  // 0 = Mission 1

const MissionProgram &activeMissionProgram()
{
  return missionPrograms[selectedMission];
}

int findMissionByName(const char *name)
{
  if (name == nullptr)
    return -1;

  for (size_t i = 0; i < MISSION_PROGRAM_COUNT; i++)
  {
    if (strcmp(missionPrograms[i].name, name) == 0)
      return (int)i;
  }

  return -1;
}

bool checkMissionNamesUnique()
{
  bool unique = true;

  // A duplicated name is invisible to the compiler and the first entry always
  // wins the lookup, so the wrong mission would run with no warning at all.
  // Comparing every pair once is ~8300 strcmp for 129 entries: a few ms, once.
  for (size_t i = 0; i < MISSION_PROGRAM_COUNT; i++)
  {
    for (size_t j = i + 1; j < MISSION_PROGRAM_COUNT; j++)
    {
      if (strcmp(missionPrograms[i].name, missionPrograms[j].name) != 0)
        continue;

      unique = false;
      Serial.print("DUPLICATE MISSION NAME: \"");
      Serial.print(missionPrograms[i].name);
      Serial.print("\" AT INDEX ");
      Serial.print(i);
      Serial.print(" AND ");
      Serial.println(j);
    }
  }

  if (unique)
  {
    Serial.print("MISSION REGISTRY OK: ");
    Serial.print(MISSION_PROGRAM_COUNT);
    Serial.println(" UNIQUE NAMES");
  }
  else
  {
    Serial.println("WARNING: FIX mission_list.cpp - THE FIRST ENTRY ALWAYS WINS");
  }

  return unique;
}
